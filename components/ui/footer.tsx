...
function Footer({ border = false }: { border?: boolean }) {
  ...
  return (
    <footer className="will-change-auto">
      ...
      {/* Big text */}
      <div className="relative -mt-16 h-60 w-full overflow-hidden pointer-events-none" aria-hidden="true">
        <div className="absolute left-1/2 -translate-x-1/2 text-center text-[348px] font-bold leading-none">
          <span className="bg-gradient-to-b from-gray-200 to-gray-100/30 dark:from-gray-800 dark:to-gray-900/50 bg-clip-text text-transparent">
            Saldopedia
          </span>
        </div>
        {/* Glow */}
        <div className="absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-2/3">
          <div className="h-56 w-56 rounded-full border-[20px] border-blue-700 dark:border-blue-500/60 blur-[80px] will-change-[filter]"></div>
        </div>
      </div>
    </footer>
  );
