...
### UI/UX Decisions
The platform features a minimalist, flat design with a blue-centric color palette, utilizing PayPal Sans fonts and a dim dark mode. It prioritizes generous whitespace, subtle backgrounds, minimal borders, and color-coded transactions (green for incoming, red for outgoing). Reusable components, AOS for animations, and Tailwind CSS for mobile-first styling are central to the design system. Saldo displays are always shown with two decimal places, and critical information includes click-to-copy functionality.
