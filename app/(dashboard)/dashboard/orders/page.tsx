...
import { orders, deposits, withdrawals, saldoTransfers, users } from '@/shared/schema';
...
export default async function OrdersPage({
  ...
  const [orderCounts, depositCounts, withdrawalCounts, transferCounts] = await Promise.all([
    db.select({
    ...
    db.select({
      ...
      completed: sql<number>`COUNT(*) FILTER (WHERE status = 'completed')::int`,
      pending: sql<number>`COUNT(*) FILTER (WHERE status IN ('pending', 'pending_proof') AND (expires_at IS NULL OR expires_at > NOW()))::int`,
  ...
  const counts = orderCounts[0] || { completed: 0, expiredDb: 0, pendingActive: 0, expiredByTime: 0 };
  const depCounts = depositCounts[0] || { total: 0, completed: 0, pending: 0, expired: 0 };
  ...
  const totalOrderCount = completedCount + pendingCount + expiredCount;
  ...
  const totalAllCount = totalOrderCount + depositCount + withdrawalCount + transferCount;
  const filteredCount = currentFilter === 'all' ? totalAllCount :
                        ...
                        currentFilter === 'transfer' ? transferCount : totalOrderCount;
  ...
  if (currentFilter === 'all') {
    ...
    const [allOrders, allDeposits, allWithdrawals, rawAllTransfers] = await Promise.all([
  ...
  const serializedOrders = userOrders.map(order => {
    ...
    const actualStatus = computeActualStatus(order.status, expiresAtDate);
