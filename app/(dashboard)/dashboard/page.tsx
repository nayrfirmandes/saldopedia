...
function computeOrderStatus(status: string, expiresAt: Date | string | null): string {
...
export default async function DashboardPage() {
  ...
  const [userOrders, userDeposits, userWithdrawals, userTransfers] = await Promise.all([
  ...
  const serializedOrders = userOrders.map(order => {
    ...
    const actualStatus = computeOrderStatus(order.status, expiresAtDate);
