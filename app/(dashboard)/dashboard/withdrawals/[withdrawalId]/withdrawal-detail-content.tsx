    ...
    <div className="max-w-2xl mx-auto space-y-8">
      ...
      <div className="border-t border-gray-200 dark:border-gray-800 pt-8 text-center">
        ...
        {(() => {
          ...
          const amountSign = isRejected ? '' : '-';
          return (
            <p className={`text-3xl font-bold ${amountColorClass}`}>
              {amountSign}{formatIDR(withdrawal.amount)}
            </p>
          );
        })()}
      </div>
      ...
      <div className="border-t border-gray-200 dark:border-gray-800 pt-8 space-y-4">
        <div className="flex justify-between py-2 border-b border-gray-100 dark:border-gray-700">
          ...
          <span className="text-sm font-medium text-gray-900 dark:text-white">{formatIDR(withdrawal.amount)}</span>
        </div>
        <div className="flex justify-between py-2 border-b border-gray-100 dark:border-gray-700">
          <span className="text-sm text-gray-500 dark:text-gray-400">{t('dashboardPages.withdrawDetail.adminFee')}</span>
          <span className="text-sm font-medium text-gray-900 dark:text-white">{formatIDR(withdrawal.fee)}</span>
        </div>
        <div className="flex justify-between py-2 border-b border-gray-100 dark:border-gray-700">
          <span className="text-sm text-gray-500 dark:text-gray-400">{t('dashboardPages.withdrawDetail.amountReceived')}</span>
          <span className="text-sm font-medium text-green-600 dark:text-green-400">{formatIDR(withdrawal.netAmount)}</span>
        </div>
        <div className="flex justify-between py-2 border-b border-gray-100 dark:border-gray-700">
